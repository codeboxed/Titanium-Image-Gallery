...
@implementation TitaniumImageGalleryObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_titaniumimagegallery);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any TitaniumImageGallery patches in JS before a context is loaded... nice for 
...
{
	// create TitaniumImageGallery global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_titaniumimagegallery = [[TitaniumImageGalleryObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_titaniumimagegallery];
	
	NSString *_titaniumimagegalleryNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _titaniumimagegalleryNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_titaniumimagegallery valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.titaniumimagegallery.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
